...
BOOL CppSQLite3DB::Open_Optimization(CString FileName)
{
	...
	char * sqlDBSet = (char*)("PRAGMA synchronous=OFF;	 PRAGMA count_changes=OFF; PRAGMA temp_store=memory; PRAGMA journal_mode = OFF");
...
int CppSQLite3DB::execScalar(LPCTSTR szSQL)
{
	...
	if (q.eof() || q.numFields() < 1)
		throw CppSQLite3Exception(CPPSQLITE_ERROR, (LPTSTR)("Invalid scalar query"),	DONT_DELETE_MSG);
...
{
	if (!mpDB)
		throw CppSQLite3Exception(CPPSQLITE_ERROR, (LPTSTR)("Database not open"), DONT_DELETE_MSG);
...
void CppSQLite3Statement::bind(int nParam, LPCTSTR szValue)
{
	...
	if (nRes != SQLITE_OK)
		throw CppSQLite3Exception(nRes, (LPTSTR)("Error binding string param"), DONT_DELETE_MSG);
}
...
void CppSQLite3Statement::bind(int nParam, const int nValue)
{
	...
	if (nRes != SQLITE_OK)
		throw CppSQLite3Exception(nRes, (LPTSTR)("Error binding int param"), DONT_DELETE_MSG);
}
...
void CppSQLite3Statement::bind(int nParam, const double dValue)
{
	...
	if (nRes != SQLITE_OK)
		throw CppSQLite3Exception(nRes, (LPTSTR)("Error binding double param"), DONT_DELETE_MSG);
}
...
void CppSQLite3Statement::bind(int nParam, const unsigned char* blobValue, int nLen)
{
	...
	if (nRes != SQLITE_OK)
		throw CppSQLite3Exception(nRes, (LPTSTR)("Error binding blob param"),DONT_DELETE_MSG);
}
...
void CppSQLite3Statement::bindNull(int nParam)
{
	...
	if (nRes != SQLITE_OK)
  		throw CppSQLite3Exception(nRes, (LPTSTR)("Error binding NULL param"),DONT_DELETE_MSG);
}
...
{
	if (mpDB == 0) throw CppSQLite3Exception(CPPSQLITE_ERROR, (LPTSTR)("Database not open"), DONT_DELETE_MSG);
}

void CppSQLite3Statement::checkVM()
{
	if (mpVM == 0)
		throw CppSQLite3Exception(CPPSQLITE_ERROR, (LPTSTR)("Null Virtual Machine pointer"), DONT_DELETE_MSG);
}
...
LPCTSTR CppSQLite3Query::fieldValue(int nField)
{
	...
	if (nField < 0 || nField > mnCols-1)
		throw CppSQLite3Exception(CPPSQLITE_ERROR, (LPTSTR)("Invalid field index requested"),DONT_DELETE_MSG);
...
int CppSQLite3Query::fieldIndex(LPCTSTR szField)
{
	...
	throw CppSQLite3Exception(CPPSQLITE_ERROR, (LPTSTR)("Invalid field name requested"),DONT_DELETE_MSG);
}
...
const char* CppSQLite3Table::fieldValue(int nField)
{
	...
	if (nField < 0 || nField > mnCols-1)
	{
		throw CppSQLite3Exception(CPPSQLITE_ERROR,
			(LPTSTR)("Invalid field index requested"),
			DONT_DELETE_MSG);
	}
...
const char* CppSQLite3Table::fieldValue(const char* szField)
{
	...
	throw CppSQLite3Exception(CPPSQLITE_ERROR,
		(LPTSTR)("Invalid field name requested"),
		DONT_DELETE_MSG);
}
...
void CppSQLite3Table::setRow(int nRow)
{
	...
	{
		throw CppSQLite3Exception(CPPSQLITE_ERROR,
			(LPTSTR)("Invalid row index requested"),
			DONT_DELETE_MSG);
	}
